...
def register_common_fgens():
    ...
    num_cols = [f'{pref}_u_p_{period}'  for pref in ['views', 'clicks', 'purchases', 'interactions', 'ctr', 'purchase_view_ratio']
    ...
    def generate_time_window_features(
        ...
        combined_df = pl.concat([history_df, target_df], how='diagonal')
        for interval in ['1w', '1mo', '3mo']:
            combined_df = (
                combined_df
                ...
                .agg([
                    (pl.col("action_type") == "AT_View").sum().alias('views_u_p_' + interval),
                    (pl.col("action_type") == "AT_Click").sum().alias('clicks_u_p_' + interval),
                    pl.col("action_type").is_in(["AT_CartUpdate", "AT_Purchase"]).sum().alias('purchases_u_p' + interval),
                    pl.lit(1).sum().alias('interactions_u_p' + interval)
                ])
                .with_columns([
                    (pl.col('clicks') / pl.col('views').cast(pl.Float64)).alias('ctr_u_p_' + interval),
                    ((pl.col('cart_updates') + pl.col('cart_updates')) / pl.col('views').cast(pl.Float64)).alias('purchase_view_ratio_u_p_' + interval),
                ])
            )
        
        return combined_df.tail(target_df.height)
