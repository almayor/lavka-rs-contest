...
def register_common_fgens_v2():
    ...
    @FeatureFactory.register(
        ...
        num_cols=[f'{name}_{pair_label}'
                  for pair_label in PAIRS.keys()
                  for name in ['days_since_interaction_v2', 'mean_days_since_interaction_v2']]
    )
    def generate_recency(
        ...
        for label, cols in PAIRS.items():
            ...
            combo_lf = (
                combo_lf
                .with_columns(recency_days_expr.alias(f"days_since_interaction_v2_{label}"))
                .with_columns(
                    pl.col(f"days_since_interaction_v2_{label}")
                        .drop_nulls()
                        .mean()
                        .over(cols)
                        .alias(f"mean_days_since_interaction_v2_{label}")
                )
            )
    ...
    @FeatureFactory.register(
        'product_temporal_patterns_v2',
        num_cols=['avg_purchase_hour_v2', 'std_purchase_hour_v2', 
                  'most_common_purchase_day_v2',
                  'hour_relevance_v2', 
                  'day_of_week_relevance_v2']
    )
    def generate_product_temporal_patterns(
        ...
        hour_stats = purchases.group_by("product_id").agg([
            pl.mean("hour_of_day").alias("avg_purchase_hour_v2"),
            pl.std ("hour_of_day").alias("std_purchase_hour_v2"),
        ])
        common_day = (
            purchases.group_by("product_id")
                .agg(
                    pl.col("day_of_week")
                        ...
                        .first()
                        .cast(pl.Int32)
                        .alias("most_common_purchase_day_v2")
                )
        )
        ...
        target_df.join(temporal_stats, on="product_id", how="left")
                 ...
                 .with_columns([
                     (pl.col("cur_hour") - pl.col("avg_purchase_hour_v2")).abs().alias("h_diff"),
                     (pl.col("cur_dow")  - pl.col("most_common_purchase_day_v2")).abs().alias("d_diff"),
                 ])
                 .with_columns([
                 ...
                 .with_columns([
                     (1 - pl.col("h_dist") / 12).alias("hour_relevance_v2"),
                     (1 - pl.col("d_dist") / 3.5).alias("day_of_week_relevance_v2"),
                 ])
                 # drop helpers
    ...
    @FeatureFactory.register(
        'cross_features_v2',
        num_cols=[
            'user_product_purchase_cross_v2',
            'user_store_purchase_cross_v2',
            'user_product_store_cross_v2'
        ],
