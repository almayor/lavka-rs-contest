...
def register_text_embedding_fgens():
    ...
    def generate_text_diversity_features(
        ...
        user_stats_list = []
        for user_id_key, group_df in user_history_with_embeds.group_by('user_id'):
            scalar_user_id_val: Any
            if isinstance(user_id_key, tuple): # Check for tuple first
                if len(user_id_key) == 1: scalar_user_id_val = user_id_key[0]
                else: logger.error(f"User ID key is multi-element tuple: {user_id_key}. Skipping."); continue
            elif isinstance(user_id_key, list): 
                if len(user_id_key) == 1: scalar_user_id_val = user_id_key[0]
                else: logger.error(f"User ID key is multi-element list: {user_id_key}. Skipping."); continue
            else: scalar_user_id_val = user_id_key
            ...
            centroid_val_list = None # Store as list
            diversity_val = 0.0
            if user_embeds_np.size > 0:
                ...
                if user_embeds_np.shape[0] > 1:
                     distances = np.linalg.norm(user_embeds_np - centroid_np_arr, axis=1) # Use centroid_np_arr here
                     diversity_val = np.mean(distances)
            
            user_stats_list.append({
                'user_id': user_id_as_string, 
                'embedding_centroid_list': centroid_val_list, 
                'embedding_diversity': diversity_val
            })
        ...
        # Preserve original dtypes for user_id and product_id from target_df
        original_target_user_id_dtype = target_df['user_id'].dtype
        original_target_product_id_dtype = target_df['product_id'].dtype

        for row_d in target_with_user_stats.iter_rows(named=True):
            dist_centroid = 0.0
            rel_diversity = 0.0
            
            output_user_id = row_d['user_id'] # This is the original user_id from target_df
            output_product_id = row_d['product_id']
            ...
            if any(x is None for x in target_emb_list) or centroid_list is None:
                diversity_features_list.append({'user_id': output_user_id, 'product_id': output_product_id,
                                                'distance_from_centroid': dist_centroid, 'relative_diversity': rel_diversity})
                continue
            
            target_emb_np = np.array(target_emb_list)
            centroid_np = np.array(centroid_list)
            ...
            if target_emb_np.shape == centroid_np.shape and target_emb_np.size > 0 and centroid_np.size > 0 : 
                ...
                if user_diversity_val > 1e-9: 
                    rel_diversity = dist_centroid / user_diversity_val
                else: 
                    rel_diversity = dist_centroid if dist_centroid > 1e-9 else 0.0 
        ...
        div_features_schema = {'user_id': original_target_user_id_dtype, 'product_id': original_target_product_id_dtype}
